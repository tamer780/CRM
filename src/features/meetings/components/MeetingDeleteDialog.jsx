...
const MeetingDeleteDialog = ({
	...
	const leadLabel =
		meeting.lead?.name ??
		meeting.lead?.phone ??
		(meeting.lead_id ? `#${meeting.lead_id}` : t("meetings.delete.fallback"));
			...
			>
				<div className="px-5 py-5">
					...
					<p className="mt-1.5 text-sm text-muted">
						{t("meetings.delete.message")}
					</p>
					<p className="mt-2 text-sm font-medium text-text">{leadLabel}</p>
					{meeting.notes && (
						<p className="mt-1 truncate text-xs text-muted">{meeting.notes}</p>
					)}
					<p className="mt-1 text-xs text-muted">
						{t("meetings.delete.cannotUndo")}
					</p>
					{error && (
						<p
							role="alert"
