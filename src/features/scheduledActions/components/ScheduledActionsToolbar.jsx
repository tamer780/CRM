...
import { hasActiveScheduledActionFilters } from "../utils/scheduledActionFilters";
...
const ScheduledActionsToolbar = ({
	...
	const hasActive = hasActiveScheduledActionFilters({
		...filters,
	...
	const statusOpts = useMemo(
		() =>
			SCHEDULED_ACTION_STATUSES.map((status) => ({
				value: status,
				label: t(`scheduledActions.statuses.${status}`),
			})),
		[t],
	);
			...
			<div className="flex flex-wrap items-end gap-2 sm:gap-3">
				<label className="relative min-w-[12rem] flex-1 basis-full sm:basis-64">
					<span className="sr-only">
						{t("scheduledActions.searchPlaceholder")}
					</span>
					<Search
					...
					<input
						...
						placeholder={t("scheduledActions.searchPlaceholder")}
				...
				<FilterDropdown
					label={t("scheduledActions.filters.type")}
					value={filters.type}
					onChange={(v) => set("type", v)}
					options={typeOpts}
					allLabel={t("scheduledActions.filters.allTypes")}
					placeholder={t("scheduledActions.filters.type")}
				/>

				<FilterDropdown
					label={t("scheduledActions.filters.status")}
					value={filters.status}
					onChange={(v) => set("status", v)}
					options={statusOpts}
					allLabel={t("scheduledActions.filters.allStatuses")}
					placeholder={t("scheduledActions.filters.status")}
				/>

				<FilterDropdown
					label={t("scheduledActions.filters.assignee")}
					value={filters.assignedId}
					onChange={(v) => set("assignedId", v)}
					options={userOpts}
					allLabel={t("scheduledActions.filters.allAssignees")}
					placeholder={t("scheduledActions.filters.assignee")}
				/>
				...
				{hasActive && (
					...
					>
						...
						{t("scheduledActions.resetFilters")}
					</button>
				)}
			</div>
		</div>
...
export default ScheduledActionsToolbar;
