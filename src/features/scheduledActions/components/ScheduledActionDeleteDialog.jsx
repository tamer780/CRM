...
const ScheduledActionDeleteDialog = ({
	open,
	action,
	isSubmitting = false,
	error = "",
	...
	const typeLabel = action.type
		? t(`scheduledActions.types.${action.type}`, { defaultValue: action.type })
		: `#${action.id}`;
			...
			>
				<div className="px-5 py-5">
					...
					<h2 id={titleId} className="text-lg font-semibold text-text">
						{t("scheduledActions.delete.title")}
					</h2>
					<p className="mt-1.5 text-sm text-muted">
						{t("scheduledActions.delete.message")}
					</p>
					<p className="mt-2 text-sm font-medium text-text">{typeLabel}</p>
					{action.note && (
						<p className="mt-1 truncate text-xs text-muted">{action.note}</p>
					)}
					<p className="mt-1 text-xs text-muted">
						{t("scheduledActions.delete.cannotUndo")}
					</p>
					{error && (
						<p
							role="alert"
				...
				<div className="flex flex-col-reverse gap-2 border-t border-border px-5 py-4 sm:flex-row sm:justify-end">
					...
					>
						{isSubmitting
							? t("common.loading")
							: t("scheduledActions.delete.confirm")}
					</button>
				</div>
			</div>
		</div>
...
export default ScheduledActionDeleteDialog;
