...
import { hasActiveProjectFilters } from "../utils/projectFilters";
...
const ProjectsToolbar = ({
	...
	const hasActive = hasActiveProjectFilters({
		...filters,
		...
		<div className="sticky top-0 z-20 rounded-2xl border border-border bg-surface/95 px-3 py-3 shadow-sm backdrop-blur-sm sm:px-4">
			<div className="flex flex-wrap items-end gap-2 sm:gap-3">
				...
				<FilterDropdown
					...
					options={statusOpts}
					allLabel={t("projects.filters.allStatuses")}
					placeholder={t("projects.filters.status")}
				/>

				<FilterDropdown
					label={t("projects.filters.team")}
					value={filters.teamId}
					onChange={(v) => set("teamId", v)}
					options={teamOpts}
					allLabel={t("projects.filters.allTeams")}
					placeholder={t("projects.filters.team")}
				/>

				<div className="flex min-w-[12rem] flex-1 flex-wrap gap-2 sm:flex-none">
					<label className="min-w-[8rem] flex-1">
						<span className="mb-1 block text-xs font-medium text-muted">
							{t("projects.filters.dateFrom")}
						</span>
						<input
							type="date"
