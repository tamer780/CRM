...
import { hasActiveAuditLogFilters } from "../utils/auditLogFilters";
...
const AuditLogsToolbar = ({
	...
	const hasActive = hasActiveAuditLogFilters({
		...filters,
		...
		<div className="sticky top-0 z-20 rounded-2xl border border-border bg-surface/95 px-3 py-3 shadow-sm backdrop-blur-sm sm:px-4">
			<div className="flex flex-wrap items-end gap-2 sm:gap-3">
				<label className="relative min-w-[12rem] flex-1 basis-full sm:basis-64">
					<span className="sr-only">{t("auditLogs.searchPlaceholder")}</span>
					<Search
				...
				<FilterDropdown
					...
					options={actionOpts}
					allLabel={t("auditLogs.filters.allActions")}
				...
				<FilterDropdown
					...
					options={modelOpts}
					allLabel={t("auditLogs.filters.allModels")}
					placeholder={t("auditLogs.filters.model")}
				/>
				...
				<FilterDropdown
					...
					options={userOpts}
					allLabel={t("auditLogs.filters.allUsers")}
