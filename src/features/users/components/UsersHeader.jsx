		...
		<div className="space-y-5">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
				<div className="min-w-0">
					...
					<p className="mt-1.5 max-w-xl text-muted">{t("users.subtitle")}</p>
				</div>
			...
			{isLoading ? (
				...
				<div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
					...
					<StatsCard
						icon={UserCheck}
						title={t("users.kpis.active")}
						value={kpis.active}
						description={kpis.activeShare}
					/>
					<StatsCard
						icon={UserMinus}
						title={t("users.kpis.inactive")}
						value={kpis.inactive}
						description={kpis.inactiveShare}
					/>
					<StatsCard
						icon={Shield}
						title={t("users.kpis.admins")}
						value={kpis.admins}
						description={kpis.adminsShare}
					/>
				</div>
			)}
		</div>
